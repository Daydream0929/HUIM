	...
	public void runAlgorithm(String input, String output, int minUtility) throws IOException {
		...
		try {
			...
			while ((thisLine = myInput.readLine()) != null) {
				...
				List<Alg.Pair> revisedTransaction = new ArrayList<>();
				// for each item
				for(int i=0; i <items.length; i++){
					/// convert values to integers
					Alg.Pair pair = new Alg.Pair();
				...
				for(int i = 0; i< revisedTransaction.size(); i++){
					Alg.Pair pair =  revisedTransaction.get(i);
...
//					int remain = remainingUtility; // FOR OPTIMIZATION
					...
					for(int j = i+1; j< revisedTransaction.size(); j++){
						Alg.Pair pairAfter = revisedTransaction.get(j);
	 ...
	 * @param item prefix to be writent o the output file
	 * @param prefix item to be appended to the prefix
